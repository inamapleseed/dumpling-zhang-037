...
// 		}
	...
	private function validate() {
		...
		if ((utf8_strlen(trim($this->request->post['unit_no'])) < 1) || (utf8_strlen(trim($this->request->post['unit_no'])) > 8)) 
			$this->error['unit_no'] = 'Unit no. must be at least 1 character.';

		//Validate birthday
		$event_day_status = $this->config->get('setting_birthdaycoupon_status');
		if(isset($event_day_status) && $event_day_status == 1){
			if ((utf8_strlen(trim($this->request->post['birthday'])) < 1) || time() < strtotime('+18 years', strtotime($this->request->post['birthday'])) || $this->request->post['birthday'] == '0000-00-00'){
