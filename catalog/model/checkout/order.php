	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'lalamove_time'           	=> $order_query->row['lalamove_delivery_time'],
			);
		} else {
			return false;
		}
		...
		if ($order_info) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW()");
                        ...
                        if($order_info['shipping_code'] == "lalamove.lalamove"){
                            if(in_array($order_status_id, $this->config->get('config_lalamove_status'))){
                                $order_status = $this->db->query("SELECT name FROM ".DB_PREFIX."order_status WHERE order_status_id = '".$order_status_id."'");

                                $shipping_info = $this->db->query("SELECT * FROM ".DB_PREFIX."lalamove WHERE lalamove_order_id = '".$order_info['order_id']."'");
                                ...
                                if($order_info['lalamove_time'] != ""){
                                    $time = $order_info['lalamove_time'];
                                }else{
                                    $time = 'H:i:s';
                                }
