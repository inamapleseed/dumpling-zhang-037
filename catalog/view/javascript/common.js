...
$(document).ready(function() {
	...
	$('.search-custom button').on('click', function(){
		...
		if (value) {
			url += '&search=' + encodeURIComponent(value).replace(/\(/g, "%28").replace(/\)/g, "%29") + '&tag=' + encodeURIComponent(value);
		}
...
/* minified */
function postalcode(e,a,l,s){if($(e).length){var r=null;$(e).on("keyup",function(t){if(t.preventDefault(),t.stopPropagation(),Number($(e).val())&&6==$(e).val().length){var n=$(e);if(1==n.length)"INPUT"==$(n[0]).prop("tagName")&&(n.parent().addClass("relative"),n.parent().append('<i class="loading-prefix-js fa fa-spinner fa-pulse absolute-center-right"></i>'));r&&r.abort(),r=$.get("https://developers.onemap.sg/commonapi/search?returnGeom=Y&getAddrDetails=Y&pageNum=1&searchVal="+$(e).val(),function(e){"results"in e&&e.found>0&&(block=e.results[0].BLK_NO,street=e.results[0].ROAD_NAME,building=e.results[0].BUILDING,a&&l&&s?($(a).val(street),$(l).val(building),$(s).val(block)):a&&l?($(a).val(block+" "+street),$(l).val(building)):a&&(address=e.results[0].ADDRESS,address_only=address.replace(e.results[0].POSTAL,""),$(a).val(address_only))),$(".loading-prefix-js").remove()})}})}$(".loading-prefix-js").remove()}
