<div class="newsletter-module<?= $uniqid ?> newsletter-module">
    <form id="form-newsletter<?= $uniqid ?>">
        <div class=""><div class="f60 orange pd-b15"><?= $title ?></div></div>
        <?php if($description) { ?>
        <div class="pd-b15 "><?= $description ?></div>
        <?php } ?>
		
		<?php if($has_subscribed) { ?>
		<div class=""><?= $text_has_subscribed ?></div>
		<?php }else{ ?>
		<div class="flex mg-b30 input-wrap">
			...
			<div class=" text-sm-right"><button type="button" id="submit-newsletter<?= $uniqid ?>" data-loading-text="<?= $text_loading; ?>" class="btn"><img src="image/catalog/slicing/footer/arrow.png" alt="image"></button></div>
		</div>
		<?php } ?>
	</form>
</div>
