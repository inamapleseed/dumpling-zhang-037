...
<div class="container">
	...
	<ul class="breadcrumb">
      ...
      <?php endif ?>
	    ...
	    <li><a href="index.php?route=product/category"><?=$lan_id == 2 ? '所有产品' : 'All Products'; ?></a></li>

    <?php foreach ($breadcrumbs as $i => $breadcrumb) { ?>
      <?php if ($i == 1): ?>
        <li class="last"><a href="<?= $breadcrumb['href']; ?>"><?= $heading_title; ?></a></li>
      <?php endif ?>
    <?php } ?>
...
<style type="text/css">
	...
	.button-share .input-group-flex > span {
		white-space: nowrap;
	}
...
<script type="text/javascript">
	let h2 = $(".product-product #content h2").text()
	$(".page-banner-title h2").empty().text(h2);
	// $(".page-banner-title ul .last a").empty().append(h2);
	// $(".page-banner-title .last a").empty();
</script>
