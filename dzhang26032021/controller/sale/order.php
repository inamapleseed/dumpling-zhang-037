...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data['pickpacklist'] = $this->url->link('sale/order/pickPackList', 'token=' . $this->session->data['token'], true);
		...
		foreach ($results as $result) {
			...
			$data['orders'][] = array(
				...
				'delivery_date' => $result['delivery_date']!= '0000-00-00'?date($this->language->get('date_format_short'), strtotime($result['delivery_date'])):'N/A',
				'delivery_time' => $result['delivery_time']?$result['delivery_time']:'N/A',
				'date_added'    => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'date_modified' => date($this->language->get('date_format_short'), strtotime($result['date_modified'])),
				'show_shipping' => $show_shipping,
				'lalamove_status' => $this->config->get('lalamove_status'),
		...
		$data['column_delivery_time'] = $this->language->get('column_delivery_time');
		...
		$data['sort_delivery_time'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=o.delivery_time' . $url, true);
		...
		$data['delivery_times'] = $this->model_sale_order->getDeliveryTimes();
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			$data['payment_unit_no'] = $order_info['payment_unit_no'];
			...
			$data['shipping_unit_no'] = $order_info['shipping_unit_no'];
	...
	public function info() {
		...
		if ($order_info) { 
			...
			$data['text_loading'] = $this->language->get('text_loading');
			$data['text_reward_allocated'] = $this->language->get('text_reward_allocated');
			$data['text_reward_unallocated'] = $this->language->get('text_reward_unallocated');
				...
				foreach ($options as $option) {
					if ($option['type'] != 'file') {
						$option_data[] = array(
							...
							'value' => $option['value'],
							'type'  => $option['type'],
							'price' => $option['price'] > 0 ? ' ('.$option['price_prefix'].$this->currency->format($option['price'], $order_info['currency_code'], $order_info['currency_value']).')' : '',
							//'price_prefix' => $option['price_prefix'],
							// 'price'  => $this->currency->format($option['price'], $order_info['currency_code'], $order_info['currency_value']),
						    // 'price_prefix'  => $option['price_prefix'],
						);
					} else {
			...
			$data['reward'] = $order_info['reward_earn'];
	...
	public function addReward() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['customer_id'] && ($order_info['reward_earn'] > 0)) {
				$this->load->model('customer/customer');

				$reward_total = $this->model_customer_customer->getTotalCustomerRewardsByOrderId($order_id);

				if (!$reward_total) {
					$this->model_customer_customer->addReward($order_info['customer_id'], $this->language->get('text_order_id') . ' #' . $order_id, $order_info['reward_earn'], $order_id);
				}
			}
				...
				foreach ($products as $product) {
					...
					if ($product_info) {
						...
						$product_data[] = array(
							...
							'price'    => $this->currency->format($product_info['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
							'total'    => $this->currency->format( ($product_info['price'] * $product['quantity'])  + ($this->config->get('config_tax') ? ($product['tax'] * $product_info['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
						);
					}
				}
	...
	public function pickPackList() {
		...
		$data['title'] = $this->language->get('text_pickpacklist');
		...
		$this->response->setOutput($this->load->view('sale/order_pick_pack_list', $data));
	}

	public function status_change_reward() {
		...
		if(in_array($order_status_id, $reward_order_status)) {
			$json['success'] = true;
		}
