...
<div id="content">
  ...
  <div class="container-fluid">
    ...
    <ul class="nav nav-tabs">
      ...
      <li><a href="#tab-instruction" data-toggle="tab"><?php echo $label_instruction; ?></a></li>
      <li>
    ...
    <div class="tab-content">
      ...
      <div class="tab-pane fade" id="tab-instruction">
        ...
        <div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> For PayNow QR in test environment, you will not be able to test it with actual PayNow app. You are required to manually change the payment status on Omise dashboard in order to trigger a success/failure payment
          <button type="button" class="close" data-dismiss="alert">&times;</button>
        </div>
        <div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> Kindly make sure customer has signed up for Omise PayNow during registration
          <button type="button" class="close" data-dismiss="alert">&times;</button>
        </div>
        <div class="panel panel-default">
          <div class="panel-heading">
            <h3 class="panel-title"><i class="fa fa-pencil"></i> <?php echo $label_title_instruction; ?></h3>
          </div>

          <div class="panel-body">
            <ol>
              <li>Register an Omise account on <a href="https://www.omise.co/">https://www.omise.co/</a></li>
              <li>
                Setup webhook URL: <a href="javascript:copyText('<?=$webhook_url?>')">Click to copy</a> <br>
                Go to Omise Dashboard and click on Webhooks <br>
                <img src="view/image/payment/omise_001.png" class="img-thumbnail img-responsive" /> <br>
                <img src="view/image/payment/omise_002.png" class="img-thumbnail img-responsive" /> <br>
              </li>
              <li>
                Copy the public and private keys and update into Omise Keys Config<br>
                <img src="view/image/payment/omise_003.png" class="img-thumbnail img-responsive" /> <br>
                <img src="view/image/payment/omise_004.png" class="img-thumbnail img-responsive" /> <br>
              </li>
              <li>Enable module and test mode</li>
              <li>
                Things to get ready before go live:
                <ul>
                  <li>Filled up live keys</li>
                  <li>Select live mode<br>
                    <img src="view/image/payment/omise_005.PNG" class="img-thumbnail img-responsive" /> <br>
                  </li>
                  <li>Change webhook url to live site (If you are in live site backend:  <a href="javascript:copyText('<?=$webhook_url?>')">Click to copy</a> )</li>
                  <li>Make sure 3DS is turned on if average payment is more than $100 <br>
                    <img src="view/image/payment/omise_006.PNG" class="img-thumbnail img-responsive" /> <br>
                  </li>
                  <li>Notes: If you want to do actual payment test on live account, kindly do refund on the same day and the transaction charges will be waived</li>
                </ul>
             </li>
                      ...
                      <form id="omise-transfer" method="post" action="<?php echo $transfer_url; ?>">
                        <div class="table-responsive">
                          <table class="table table-bordered table-hover table-striped">
                            ...
                            <tbody>
                              <?php foreach ($omise_dashboard['charge_data'] as $key => $value): $date = new \DateTime($value['created_at']); ?>
                                <tr>
                                  <td><strong class="<?php echo ($value['failure_code']) ? 'text-danger' : ((!$value['authorized'] || !$value['capture']) ? 'text-warning' : 'text-success'); ?>"><?php echo OmisePluginHelperCurrency::format($omise_dashboard['currency'], $value['amount']); ?></strong></td>
                                  ...
                                  <td><?php echo $value['capture'] && $value['authorized']? '<strong class="text-success">Yes</strong>' : 'No'; ?></td>
                      ...
                      <form id="omise-transfer" method="post" action="<?php echo $transfer_url; ?>">
                        ...
                        <div class="table-responsive">
                          <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                              <?php foreach ($omise_dashboard['transfer_data'] as $key => $value): $date = new \DateTime($value['created_at']); ?>
                                <tr>
        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-setting" class="form-horizontal">
          <div class="panel panel-default">
            ...
            <div class="panel-body">
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="omise_success_status">Success Status</label>
                <div class="col-sm-10">
                  <select class="form-control omise_success_status"name="omise_success_status" >
                    <?php foreach ($order_statuses as $order_status ) { ?>
                      <option value="<?=$order_status['order_status_id']?>" <?=$omise_success_status == $order_status['order_status_id']?'SELECTED':''?>><?=$order_status['name']?></option>
                    <?php  } ?>
                  </select>
                </div>
              </div> 
              <div class="form-group hide">
                <label class="col-sm-2 control-label" for="omise_cancel_status">Cancel Status</label>
                <div class="col-sm-10">
                  <select class="form-control omise_cancel_status"name="omise_cancel_status" >
                    <?php foreach ($order_statuses as $order_status ) { ?>
                      <option value="<?=$order_status['order_status_id']?>" <?=$omise_cancel_status == $order_status['order_status_id']?'SELECTED':''?>><?=$order_status['name']?></option>
                    <?php  } ?>
                  </select>
                </div>
              </div> 
              <div class="form-group hide">
                <label class="col-sm-2 control-label" for="omise_process_status">Processing Status</label>
                <div class="col-sm-10">
                  <select class="form-control omise_process_status"name="omise_process_status" >
                    <?php foreach ($order_statuses as $order_status ) { ?>
                      <option value="<?=$order_status['order_status_id']?>" <?=$omise_process_status == $order_status['order_status_id']?'SELECTED':''?>><?=$order_status['name']?></option>
                    <?php  } ?>
                  </select>
                </div>
              </div> 
              <div class="form-group">
                ...
                <div class="col-sm-10">
                  <input type="text" name="omise_sort_order" value="<?php echo $omise_sort_order; ?>" placeholder="Sort Order" id="input-sort-order" class="form-control" />
                </div>
              </div>
              <!-- Live mode -->
              <div class="form-group">
...
<script>
  ...
  function copyText(webhook_url) {
    var temp_input = $("<input>");
    $("body").append(temp_input);
    temp_input.val(webhook_url).select();
